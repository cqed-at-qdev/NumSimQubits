...
class Gaussian_pulse_w_DRAG:
    """ Class representing a gaussian pulse with DRAG.
    ...
    Args:
        ...
        drag_coeff (float): The coefficient for DRAG. 
    ...
    def __init__(self, t_0, sigma, amp, freq, phase, drag_coeff, t_ls, t_cutoff=4.0):
        """
    ...
    def __repr__(self):
        ...
        return 'Gaussianpulse(\n t_0=%r, \n sigma=%r, \n amp=%r, \n freq=%r, \n phase=%r, \n drag_coeff=%r, \n t_cutoff=%r \n)' % (
            self.t_0, self.sigma, self.amp, self.freq, self.phase, self.drag_coeff, self.t_cutoff)
    ...
    def update_base(self):
        ...
        drag_shape     =  -1j * self.amp * self.drag_coeff * np.exp(1j * self.phase) * 2 * (self.t_ls - self.t_0) / (2 * self.sigma ** 2) * np.exp(- (self.t_ls - self.t_0) ** 2 / (2 * self.sigma ** 2)) / norm

        gaussian_pulse  =  gaussian_shape * ((self.t_ls > self.t_0 - t_cutoff / 2) & (self.t_ls < self.t_0 + t_cutoff / 2))
        gaussian_pulse +=  drag_shape     * ((self.t_ls > self.t_0 - t_cutoff / 2) & (self.t_ls < self.t_0 + t_cutoff / 2))
